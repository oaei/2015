...
\author{Christian Meilicke, C\'assia Trojahn, Daniel Faria, \\ Ernesto Jim\'{e}nez-Ruiz, Jakob Huber and J\'er\^ome Euzenat}

\maketitle

\begin{abstract}
This tutorial explains how to prepare, package and zip an ontology matching tool to be integrated into the SEALS platform. It explains 
how to check that the generated zip-file is valid and can be executed on the platform. Further, it describes how the wrapped tool can 
be evaluated locally with one of the test suites used in the Ontology Alignment Evaluation Initiative. 
\end{abstract}
...
\textbf{bin/}   
   ...
   * (X) some *.bat or *.sh scripts
   \textbf{lib/}  * tool libs and other libs required by your tool  
\textbf{conf/}
...
% (this template contains the \tool{MyTool} matcher example, referred later in the sections of this tutorial);
...
\begin{enumerate}
	...
	\textbf{Note that the directory \file{bin/lib} should not include the file \file{seals-omt-client.jar}}.
	...
	\item Zip the content of the package \textbf{(do not include the package folder itself in the zip-file, the zip file should directly contain the folder structure presented in Section \ref{sec:intro})}.
\end{enumerate}
...
% JE: suppressed all this text which brings no help to the user
%There is no strict distinction between the two folders. However, we decided to distinguish between these two types of files, because in the next version of the platform it might be relevant.
...
% JE: suppressed all this text which brings no help to the user
%Note that the complete contents of \file{conf} and \file{lib} are copied recursively into the directory where the matcher will finally be executed.
...
Once you have tested successfully the \verb|-t| or the \verb|-o| option, it is time to test if your system manages to run a complete evaluation. 
For doing so you can use the \verb|-x| option and chose one of the available suites, which you can find at the corresponding pages for each of the OAEI tasks
\url{http://oaei.ontologymatching.org/2015}.

For example, you can run all test cases of the Conference test suite which is used for the OAEI 2015. 
The \verb|repURI| is \verb|http://repositories.seals-project.eu/tdrs/|, the \verb|suiteId| is \verb|conference| and the \verb|versionId| is \verb|conference-v1|. So, you have to type this:
\begin{verbatim}
A:\seals\sealshome>java -jar A:/temp/seals-omt-client.jar 
A:/temp/yoursystem-package -x  http://repositories.seals-project.eu/tdrs/
conference conference-v1 A:\results\
\end{verbatim}
This runs all of the test cases subsequently and displays some progress information on the screen.

Note that a full evaluation can be run successfully while there might still be some problems related to the generated alignments. 
For that reason you should inspect the output files, which you can found in the specified output directory, carefully. 
The output directory contains all generated alignments and also a summary file \verb|results.txt|, which might look like this:
\begin{verbatim}
...
%\label{notes}
%
%Two things follow from the proposed procedure: (1) Everybody can see results that other systems generate in advance and can compare his systems againts this. Knowing this, might prevent them from uploading a version of their system, becase they want to make as long as possible a secret about how good they are. (2) Results are known to the particpants previously and they can put them in their results paper, which is a good thing! This was not alawass the case in the past.
...
% CTS: we can put the link to the zip file not to the jar files. 
...
% JE: suppressed all this text which brings no help to the user
%In the future, they will allow a very flexible way to deploy a tool in the SEALS platform. 
%Since we want to be compatible with future versions of the SEALS platform we already stick to the requirement that these folders and files exist.

% JE: suppressed all this text which brings no help to the user
%Currently, we have to deal with a simple work-around. This work-around is based on the directory \file{conf} and its contents. It is possible to place in this folder \textit{resources required at runtime} like configuration files and additional resources, e.g., WordNet files. Note that additional jar-files required by the matcher have to be stored in the \file{bin/lib} folder, which is a different one with a different purpose! 
...
% JE: suppressed all this text which brings no help to the user
%Currently their execution is not supported. As already mentioned, we want to be compatible with later versions of the SEALS platform. For that reason these files have to be included.

% that does not require a specific operation to be performed in the deployment/undeployment or start/stop phase. Examples for such operations are the following ones:
%\begin{itemize}
%	\item A system requires to build up a complex index structure over a UMLS subset to be executed correctly. This is a time consuming task that will normally only be performed when installing the tool. This operation has to be called from the \file{deploy.bat}. Another example is the installation of a DB used by the matching system.
%	\item A matching system works like a server that can be started and stopped. Once the server has been started a method to match two ontologies can be called. In this setting the \file{start.bat} and \file{stop.bat} should be used to start and stop the server.
%\end{itemize}
%In the first version of this tutorial we do not focus on these issues. If such operations are required, they should be explained to the SEALS contact person, which will then assist in generating/modifying the tool package in the correct way regarding these issues. Note that these operations can be defined by adding the respective commands in the bat or shell-scripts in a standard way.
